...
window.formAccessOverride = false;

// Retourne l'utilisateur Google connecté si disponible
...
function getAuthErrorMessage(error) {
    ...
    if (code === 'auth/web-storage-unsupported') {
        return '❌ Stockage navigateur indisponible. Désactivez le mode privé et autorisez les cookies.';
    }

    if (code === 'auth/operation-not-supported-in-this-environment') {
        return '❌ Environnement non supporté. Utilisez HTTPS (ou localhost) et un navigateur standard.';
    }
...
async function signInWithGoogle() {
    console.log('🔐 Connexion Google via popup (redirect désactivé)');
    try {
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        updateAuthUI(null);
        if (window.formAccessOverride === true) {
            hideAuthGate();
        } else if (isRedirectInProgress()) {
            console.log('⏳ Redirect Google en cours, attente du résultat...');
            showAuthGate('Connexion en cours... Merci de patienter.', { disableButton: true });
        } else {
            showAuthGate('Connecte-toi pour accéder au site.');
        }
...
async function ensureGoogleAuth() {
    ...
    if (signedInUser) {
        console.log("✅ Déjà connecté avec Google:", signedInUser.email);
        console.log("🔑 UID conservé:", signedInUser.uid);
        return signedInUser;
    }

    if (isRedirectInProgress()) {
        console.log('⏳ Redirect Google déjà en cours, attente...');
        return null;
    }
...
// ========================================
// AUTH GATE (Connexion Google obligatoire)
// ========================================
...
function showAuthGate(message, options = {}) {
    ...
    if (btn) {
        btn.disabled = options.disableButton === true;
    }
...
async function showFormPage() {
    ...
    window.formAccessOverride = true;
    let signedInUser = getSignedInUser();

    // Tenter l'auth Google si pas connecté, mais ne pas bloquer l'affichage du formulaire
    if (!signedInUser) {
        try {
            ...
            console.warn("⚠️ Auth Google indisponible, ouverture du formulaire en mode simple.", error);
        }
    }

    // Si connecté, appliquer la logique de crédits
    if (signedInUser) {
...
// ========================================
// SLIDER EXEMPLES DE SITES
// ========================================

function initExamplesSlider() {
    const slides = Array.from(document.querySelectorAll('.example-slide'));
    const prevBtn = document.getElementById('examples-prev');
    const nextBtn = document.getElementById('examples-next');
